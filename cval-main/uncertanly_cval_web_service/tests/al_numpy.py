from random import randint, random

from uncertanly_cval_web_service.src.core.al._np import al
from uncertanly_cval_web_service.src.core.models import FramePrediction, BBoxScores, DetectionSamplingOnPremise

# TODO: преобразовать в нормальные тесты


frames_predictions = list(
    map(
        lambda x: FramePrediction(
            frame_id=f"{randint(0, 200)}",
            predictions=list(
                map(lambda _: BBoxScores(category_id=randint(0, 2), score=random(), probabilities=[0.3, 0.3]),
                    range(200)))
        ),
        range(200)
    )
)

request = DetectionSamplingOnPremise(
    num_of_samples=200,
    bbox_selection_policy='sum',
    selection_strategy='cval_custom',
    sort_strategy='ascending',
    frames=frames_predictions,
    probs_weights=[1, 1]
)

result = al(request)

print(al(request))
