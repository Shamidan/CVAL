...
def least_confidence(prob_dist, num_labels=2):
    if len(prob_dist) == 0:
        prob_dist = np.array([[1 / num_labels] * num_labels])
    simple_least_conf = np.max(prob_dist, 1)
...
def new_entropy(prob_dist, num_labels, w):
    if len(prob_dist) == 0:
        prob_dist = np.array([[1 / num_labels] * num_labels])
    log_probs = w * prob_dist * np.log2(prob_dist)
...
def margin_confidence(prob_dist, num_labels=2, w=None):
    if len(prob_dist) == 0:
        prob_dist = np.array([[1 / num_labels] * num_labels])
    prob_dist = -np.sort(-prob_dist, axis=1)
...
def fa2(func, preds: List[BBoxScores], w):
    ...
    return dict_strategy[func](np.array(list_value), len(w), np.array(w))
